...
class CreateComment(BaseCommentView):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                    "دیدگاه جدید",
                    "دیدگاه جدیدی برای مقاله <{}> که شما نویسنده آن هستید ارسال شده است:\n{}{}".format(
                        article, current_site, reverse(
                            'blog:post', kwargs={'slug': article.slug}
                            )
                    ),
                    to=[author_email]
            )
        ...
        if parent_email:
            email = EmailMessage(
                    "دیدگاه جدید",
                    "پاسخ جدیدی برای دیدگاه پست<{}> ارسال شده است برای مشاهده روی لینک زیر کلیک کنید:\n{}{}".format(
                        article, current_site, reverse(
                            'blog:post', kwargs={'slug': article.slug}
                            )
                    ),
